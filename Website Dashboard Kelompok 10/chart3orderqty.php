...
<body class="g-sidenav-show   bg-gray-100">
                    ...
                    <div class="card z-index-2 h-100">
                        <div class="card-header pb-0 pt-3 bg-transparent">
                            ...
                            <p class="text-sm mb-0" style="text-align: center;">
                                <span class="font-weight-bold">Berapa order quantity untuk produk sub category “Gloves”
                                    dan “Vests” dari bulan 1 - 12 tahun 2003? </span>
                            </p>
                        </div>
                        <div class="card-body p-3">
                            <div class="chart">
    ...
    <script>
        ...
        <?php
        $query = "SELECT t.month, p.product_subcategory, SUM(fp.orderqty) AS total_order_qty 
              FROM sales_fact fp 
              JOIN product_dim p ON fp.product_id = p.product_id 
              JOIN time_dim t ON fp.time_id = t.time_id 
              WHERE p.product_subcategory IN ('Gloves', 'Vests') AND t.year = 2003 
              GROUP BY t.month, p.product_subcategory 
              ORDER BY t.month";
        $result = mysqli_query($conn, $query);

        $months = range(1, 12);
        $products = ['Gloves', 'Vests'];
        $datasets = [];
        ...
        foreach ($products as $index => $product) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                if ($row['product_subcategory'] == $product) {
                    $month = $row['month'];
                    $totalOrderQty = $row['total_order_qty'];
                    $data[$month - 1] = $totalOrderQty;
                }
            }
        ...
        var options = {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Order Quantity for Gloves and Vests (2002)',
                    font: {
                        size: 16
                    }
                }
